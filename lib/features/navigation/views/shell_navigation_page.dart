...
import 'package:barpass_app/features/navigation/widgets/circular_notched_shape.dart';
import 'package:barpass_app/features/navigation/widgets/custom_bottom_appbar.dart';
import 'package:barpass_app/features/navigation/widgets/floating_action_button_widget.dart';
...
class _ShellNavigationPageState extends State<ShellNavigationPage> {
  ...
  Future<void> _onFabPressed() async {
    try {
      ...
      // Simula operação de scan QR code
      await Future<void>.delayed(const Duration(milliseconds: 800));

      if (mounted) {
        // Navegar para scanner QR
        await context.push(AppRoutes.qrScanner.path);
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Erro ao executar ação'),
